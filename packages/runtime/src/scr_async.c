...
bool scr_loop_run(ScrPromise *top_level) {
  ...
  for (;;) {
    ...
    if (scr_reffed_timers == 0 && scr_reffed_immediates == 0 && !scr_children_reffed_pending() && !io && !events && !net && !dgram && !midi && !watch && !ffi && !renames) break;
    ...
    if (io) {
       ...
       * cost. */
      else if ((evw || net || dgram || midi || watch || ffi) && due > now + SCR_SIGNAL_POLL_MS) due = now + SCR_SIGNAL_POLL_MS;
      scr_io_poll_fn(due > now ? due - now : 0);
      now = scr_now_ms();
      if (scr_ready_len > 0) continue; /* io callbacks woke fibers */
    } else if (evw || net || dgram || midi || watch || ffi) {
